...
// quote them back, not estimate its own. Reframed from a spread-savings
// pitch to a hedging/risk pitch: Grain's actual differentiator is
// absorbing 100% of the FX risk between transaction and settlement via a
// rate lock, not being marginally cheaper than a bank.
export function buildFxFollowUpPrompt(input: FxFollowUpInput): string {
  const { volume, pair, vertical, settlementDays, volatilityPct, profitAtRiskUsd, prospectName } = input;

  return `You are drafting a short follow-up email for a Grain sales rep to send after a conference-floor conversation. Grain protects travel platforms, PSPs, and cross-border businesses from currency volatility by locking exchange rates via API from transaction inception through settlement date, absorbing 100% of the FX risk in between — the prospect's margin is guaranteed the moment the transaction starts, not whatever the market happens to do by settlement.

The rep just ran a live risk estimate with this prospect on the show floor, using numbers the prospect gave in the conversation:

- Vertical: ${vertical}
- Currency pair: ${pair}
- Annual cross-border volume: $${volume.toLocaleString("en-US")}
- Typical settlement/payout delay: ${settlementDays} days
- Estimated profit at risk from unhedged ${pair} exposure over that window: $${Math.round(profitAtRiskUsd).toLocaleString("en-US")}/year (≈${volatilityPct}% of volume, an illustrative volatility proxy, not a formal quote)
- With Grain's rate lock: that exposure goes to $0 — the rate is fixed at transaction inception, not left floating until settlement
${prospectName ? `- Prospect's name: ${prospectName}` : ""}

Write a short follow-up email (70-110 words) that:
1. Names the specific risk in concrete terms: their money sits exposed to ${pair} movement for the full ${settlementDays}-day settlement window, worth roughly $${Math.round(profitAtRiskUsd).toLocaleString("en-US")}/year at their volume — this must read as tailored to this exact conversation, not generic.
2. States Grain's actual differentiator in one line: the rate is locked via API at transaction inception, so margin quoted is margin kept — not a claim about being cheaper.
3. Ends with one clear, low-friction call to action (e.g. a 15-minute call, or a formal risk assessment).
