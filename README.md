...
| **Calculator** (`/calculator`) | Bonus: an FX-savings calculator a rep can pull up mid-conversation. Not one of the 7 core requirements — kept isolated on its own page. |
...
| At least one meaningful AI feature | AI relationship-arc summarizer — `lib/ai-prompt.ts` + `app/api/ai/relationship-summary/route.ts`, surfaced in `/contacts` |
| Path to push leads into HubSpot | `app/api/hubspot/push/route.ts`, triggered per-interaction from `/contacts` |
...
## The AI feature — relationship-arc summarizer

`/contacts` → "AI relationship summary" calls Gemini (`gemini-2.0-flash`) with
the contact's full interaction history and asks for (1) a 2–3 sentence read of
how the relationship has *actually* evolved — including what the rule-based
signal alone wouldn't catch — and (2) one concrete next action.
...
the AI layer adds judgment on top when a key is configured.
...
- **Only one AI feature was built deep, not two shallow ones** — the
  relationship-arc summarizer, because it maps directly onto the hardest,
  most-weighted requirement in the brief (cross-conference intelligence). An
  AI-assisted lead-qualification scorer (reading a rep's raw field notes into
  a suggested ICP tier) is the natural second feature with another week.
